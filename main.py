...
class SolveWork(QMainWindow):
    def __init__(self, parent=None, tlist=None, count=None):
        ...
        self.pushButton_5.clicked.connect(self.work_with_word)
        self.pushButton_6.clicked.connect(self.work_with_word)
        self.con = sqlite3.connect("PhB.db")
        cur = self.con.cursor()
        a = cur.execute("select name from sqlite_master where type = 'table' and name not like '_all'").fetchall()
        for i in a:
    ...
    def on_changed(self, text):
        ...
        self.comboBox_2.activated[str].connect(self.case_changed)
        self.con.close()
        print(text)
